...
public interface FeedbackDao {
    ...
    @Query("SELECT DISTINCT courseLevel FROM course_table WHERE courseDepartment LIKE :department")
    LiveData<List<String>> getCourseLevels(String department);

    @Query("SELECT DISTINCT courseDepartment FROM course_table")
    LiveData<List<String>> getCourseDepartments();

    @Query("SELECT courseID FROM course_table WHERE courseLevel LIKE :level AND courseDepartment LIKE :department")
